...
modelPath = fullfile(root, 'logs', 'acmotor', 'model_final.mat');
if exist(modelPath, 'file') ~= 2
    error('Model not found: %s\nRun train_acmotor first.', modelPath);
end

agent = PPO(ACMotorEnv(), rlConfig('ACMotor'));
agent.load(modelPath);
res = agent.evaluate(20);
fprintf('ACMotor test: mean %.2f +/- %.2f, min %.2f, max %.2f\n', ...
